...
export default function TransactionsTable({ orders }: Props) {
  ...
  const sorted = [...orders].sort((a, b) => {
    ...
    if (sort.key === "createdAt") return (new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()) * mul;
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-5">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Latest {orders.length} orders</p>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              >
                <span className="flex items-center gap-1">Amount <SortIcon k="total" /></span>
              </th>
              <th
                className="text-left pb-3 px-2 text-[10px] font-semibold text-gray-500 uppercase tracking-widest cursor-pointer select-none hover:text-gray-700 transition-colors"
              ...
              >
                <span className="flex items-center gap-1">Date <SortIcon k="createdAt" /></span>
              </th>
            </tr>
          </thead>
