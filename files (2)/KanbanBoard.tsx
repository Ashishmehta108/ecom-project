...
function Card({ card, columnId }: { card: KanbanCard; columnId: string }) {
  const colors = columnColors[columnId as keyof typeof columnColors] || columnColors.leads;
...
function Column({
  ...
  const colors = columnColors[col.id as keyof typeof columnColors] || columnColors.leads;
...
export default function KanbanBoard() {
  ...
  const handleDrop = (toColId: string) => {
    ...
    setColumns((prev) => {
      ...
      // Add to target column if different
      ...
      // Return original if same column
      return prev;
    });
    setDragOverCol(null);
  };
