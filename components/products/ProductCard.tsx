...
export function ProductCard({ product, userId, listView }: ProductCardProps) {
  ...
  const addToCart = async (userId: string, productId: string) => {
    ...
    if (data.success) toast.success(`${data.data.name} added to cart`);
  };
  ...
  return (
    <article
      className={cn(
        ...
        listView ? "flex gap-4 p-4 h-auto" : "flex h-[540px] flex-col"
      )}
    >
      ...
      >
        ...
        <Image
          ...
          className={cn(
            "object-contain transition-transform duration-500 group-hover:scale-105 p-3",
      ...
      >
        ...
        {/* Features (only in grid mode for cleaner list layout) */}
        {!listView && (
        ...
        >
          ...
          <div className=" flex items-center ">
            {/* If discount exists, show discounted price */}
