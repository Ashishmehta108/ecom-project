...
type Product = {
  ...
  tags: string[];
  productImages: ProductImage[];
  categories: string[]; // category names (matches backend expectations)
  slug?: string;
};
...
// ---------------------------------------------------------------------------
// Tabs (General, Pricing, Specs, Images, Features, Tags, Categories)
// ---------------------------------------------------------------------------
...
const SpecsTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-3">General Specifications</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <InputFieldForSpec
          ...
          />
          <InputFieldForSpec
            label="Certificate (comma separated)"
            value={
...
const InputFieldForSpec: React.FC<{
  label: string;
  value: string;
...
// Categories Tab — uses category.name as value (backend expects names)
const CategoriesTab: React.FC<{ categories: Category[] }> = ({
  categories,
}) => {
  const form = useFormContext<ProductFormValues>();
          ...
          <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
                {isNew
                  ? "Fill in the details to create a new product listing"
                  : "Update product details, pricing, images, and categories"}
              </p>
            </div>
          </div>
          ...
          <CardContent className="p-6">
            <FormProvider {...form}>
              <Form {...form}>
                ...
                >
                  <Tabs defaultValue="general" className="space-y-6">
                    <TabsList className="grid w-full grid-cols-4 lg:grid-cols-7 gap-2 h-auto p-1 bg-muted/50">
                      <TabsTrigger value="general">General</TabsTrigger>
                      <TabsTrigger value="pricing">Pricing</TabsTrigger>
                      <TabsTrigger value="specs">Specs</TabsTrigger>
                      <TabsTrigger value="images">Images</TabsTrigger>
